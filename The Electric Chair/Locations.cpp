...
void Locations::washing()
{
	...
	else
	{
		if (i_coins > 0)
		{
			l_game.print(" You put a coin in the maschine.. ", 35); //test test
			l_game.print(" Your prisonscrubbs smell like flowers..mmm", 35);
			cout << "coins: " << i_coins << endl; // Om man inte har en uniform alls
			i_coins = i_coins - 1;
			if (i_coins <= 0)
			{
				b_coins = false;
